...
#this script is a subset of the endemism.php script
	...
	$dp = fopen('forMappingAll4_v2.txt', 'w');
...
function matrixOutput($species_id,$countingNumber,$Genus,$Species){
				global $DB;
				$matrixOutputValue = $DB->query("Select distinct L1.DLat,L1.DLong FROM Specimen S1 left join MNL T1 ON S1.Genus = T1.MNLUID left join MNL T2  ON S1.Species = T2.MNLUID left join MNL T3 ON S1.Tribe=T3.MNLUID left join MNL T4 on S1.Subfamily=T4.MNLUID left join MNL T5 on T4.ParentID=T5.MNLUID left join Locality L1 on S1.Locality=L1.LocalityUID left join Flora_MNL F1 ON S1.HostG=F1.HostMNLUID left join Flora_MNL F2 ON S1.HostSp=F2.HostMNLUID left join Flora_MNL F3 ON S1.HostSSp=F3.HostMNLUID left join Flora_MNL F4 ON S1.HostF=F4.HostMNLUID left join SubDiv SD on L1.SubDivUID=SD.SubDivUID left join StateProv SP on SD.StateProvUID=SP.StateProvUID left join colevent CE on S1.ColEventUID=CE.ColEventUID left join Collector C1 on CE.Collector=C1.CollectorUID left join Country CN on SP.CountryUID=CN.UID where T2.TaxName not like '%#%' and T2.TaxName not like '%sp.%'and T2.TaxName != 'sp' and T2.TaxName !='sp,' and T2.TaxName not like '%\_%' and T2.TaxName not like '%spp.%' and T2.TaxName not like '%undetermined%'and T2.TaxName != 'unknown' and DLat != '0.00000' and L1.LocalityStr !='unknown' and (CN.UID = '2' or CN.UID = '8' or CN.UID = '11') and (S1.Subfamily = '6295' or S1.Subfamily = '8150' or S1.Subfamily = '6294' or S1.Subfamily = '8163') and (S1.Subfamily = '6295' or S1.Subfamily = '8150' or S1.Subfamily = '6294' or S1.Subfamily = '8163') and S1.species='$species_id'");
				#(CN.UID = '2' or CN.UID = '8' or CN.UID = '11')
					...
					$Dmatrix = '';
						...
						while ($matrixRow =& $matrixOutputValue->fetchRow()){
							...
							$DLong = $matrixRow[1];
							echo $Genus . " " . $Species . "\n";
							$Dmatrix .= $Genus . "_" . $Species . "\t" . $DLong . "\t" . $DLat . "\n";
